"""approach23 v21: v4s(v4+치환스펙트럼) 기준선에 gene->pathway 모듈 피처(유형 인지 포함)를 추가해서
정직 OOF(group-twins)로 검증. test 미사용, 기존 approach4의 GENE_INFO/PATHWAYS 재사용(중복 방지).
"""
import sys
...
conditions = ["v4s_base", "v4s_plus_pathway"]
...
for k, (tri, vai) in enumerate(folds):
    ...
    m2 = xgb.XGBClassifier(**PARAMS).fit(Xtr_pw, y[tri])
    oof["v4s_plus_pathway"][vai] = m2.predict(Xva_pw)

    print(f"fold{k} 완료 (pathway 피처 {pw_tr.shape[1]}열)", flush=True)
...
f1_pw = f1_score(y, oof["v4s_plus_pathway"], average=None, labels=range(len(classes)))
...
changed = oof["v4s_plus_pathway"] != oof["v4s_base"]
print(f"예측 바뀐 행: {changed.sum()} ({changed.mean():.1%})")

out_dir = ROOT / "6. experiments" / "2026-09-14_approach23_v17_light_aug_burden_norm"
tbl.to_csv(out_dir / "v21_pathway_module_summary.csv")
diff.to_csv(out_dir / "v21_pathway_module_class_diff.csv")
print(f"\nsaved: {out_dir}")
