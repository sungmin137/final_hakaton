"""approach23 v25: v4s(v4+치환스펙트럼) 기준선에 gene_rate_gini(변이 유전자들의 모집단 rate 분포 불평등도) 1개만
추가해 정직 OOF(group-twins)로 검증. rate(g)는 각 fold의 TRAIN 부분만으로 fit(validation/test 정보 미사용).
"""
import sys
...
from approach23_synthetic_aug.gene_rate_gini import fit_gene_rate, gene_rate_gini_features
...
conditions = ["v4s_base", "v4s_plus_gene_rate_gini"]
...
for k, (tri, vai) in enumerate(folds):
    ...
    rate = fit_gene_rate(tr_part, genes)  # fold의 TRAIN 부분만 사용
    gini_tr = gene_rate_gini_features(tr_part, genes, rate).reset_index(drop=True)
    gini_va = gene_rate_gini_features(va_part, genes, rate).reset_index(drop=True)
    Xtr_g = pd.concat([Xtr_base, gini_tr], axis=1)
    Xva_g = pd.concat([Xva_base, gini_va], axis=1)
    m2 = xgb.XGBClassifier(**PARAMS).fit(Xtr_g, y[tri])
    oof["v4s_plus_gene_rate_gini"][vai] = m2.predict(Xva_g)

    imp = pd.Series(m2.feature_importances_, index=Xtr_g.columns)
    importances.append(imp[["gene_rate_gini"]])

    print(f"fold{k} 완료 (gini train으로만 fit, rate 유전자수={len(rate)})", flush=True)
...
f1_g = f1_score(y, oof["v4s_plus_gene_rate_gini"], average=None, labels=range(len(classes)))
diff = pd.Series(f1_g - f1_base, index=classes).sort_values()
print("\nclass별 변화 (26개 전체):")
print(diff.to_string())
print("\n최악5 / 최선5:")
print("  최악:", ", ".join(f"{cl}({diff[cl]:+.3f})" for cl in diff.head(5).index))
print("  최선:", ", ".join(f"{cl}({diff[cl]:+.3f})" for cl in diff.tail(5).index))
changed = oof["v4s_plus_gene_rate_gini"] != oof["v4s_base"]
print(f"\n예측 바뀐 행: {changed.sum()} ({changed.mean():.1%})")

print("\nfeature importance (gene_rate_gini), fold 평균:")
...
tbl.to_csv(out_dir / "v25_gene_rate_gini_summary.csv")
diff.to_csv(out_dir / "v25_gene_rate_gini_class_diff.csv")
print(f"\nsaved: {out_dir}")
