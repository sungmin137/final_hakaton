"""LUAD↔LUSC soft router 검증 (approach 번호 미확정, approach19 구조 재사용).

train 정직 CV 혼동행렬에서 LUAD<->LUSC 36건 확인(둘 다 폐암, 임상에서도 흔히 헷갈리는 쌍).
동일 레시피(변이율 차이 상위 40유전자, 로지스틱회귀 C=.1, 확신도 임계값 .70)로 신호 여부만 확인.
test.csv는 읽지 않는다.
"""
...
def main():
    ...
    result = {"description": "LUAD/LUSC C=.1 threshold=.7 fixed repeat SGKF; test 미사용", "rows": rows,
    ...
    out = ROOT / "6. experiments" / "2026-09-12_pending_luad_lusc_repeat"
