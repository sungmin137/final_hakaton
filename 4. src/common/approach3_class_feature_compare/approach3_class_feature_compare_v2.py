...
실행: PYTHONPATH="4. src/common" python3 "4. src/common/approach3_class_feature_compare/approach3_class_feature_compare_v2.py"
입력: v1 산출물(6. experiments/approach3_class_feature_compare_v1/oof_*.npy) + v4 정직 CV OOF
산출: 6. experiments/approach3_class_feature_compare_v2/result.json (최적 가중치, 교차확인 이득)
...
OUT = ROOT / "6. experiments" / "approach3_class_feature_compare_v2"
...
def main():
    tr = pd.read_csv(ROOT / "1. info/data/train.csv", usecols=["SUBCLASS"]); classes = sorted(tr.SUBCLASS.unique())
