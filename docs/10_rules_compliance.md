...
| 클래스 배율 후처리(`--class-scale`) | train의 정직 CV OOF 확률 + train 라벨로만 배율 결정. test 확률·분포 미사용 | `src/postprocess.py` | ✅ |
...
구현: `src/knowledge_features.py` (BLOSUM62 + 아미노산 특성 변화량). 출처: Henikoff & Henikoff 1992 (BLOSUM62), Kyte & Doolittle 1982 (소수성), Zamyatnin 1972 (잔기 부피).
