...
|---|---|---|---|
| v1 | approach5_v1_20260909_2135 | v5 단독 0.4761 → +접근3 블렌딩 0.4916(낙관) → +클래스 배율 | **0.41217** |
| v2 | approach5_v2_20260909_2322 | v5 피처 + 클래스 배율만 (접근3 블렌딩 제외). v1과 LB 비교용 | **0.41211** |

## 주의
- 접근 3 블렌딩은 CV에서 +0.013이었지만 LB에서는 역효과(5차 0.417 < 3차 0.43)였다. 통합 케이스의 LB가 접근 2 v2(0.43)보다 낮으면 블렌딩을 빼는 v2를 만든다.

## 리더보드 결과 (2026-09-10)
- v1 0.41217, v2 0.41211 → 블렌딩 유무는 LB에 영향 없음(차이 0.00006). 둘 다 접근2 v2(0.43)보다 낮다.
- 즉 **문헌 피처(접근 4)를 더한 v5는 정직 CV +0.008이었지만 LB −0.018**. v4 이후의 모든 추가는 LB를 낮췄다.
- 최종 제출 기준선은 여전히 approach2_v2_20260909_1653 (LB 0.43).
