...
- py 파일(`src/submissions/`)과 제출 csv(`submissions/`)는 **같은 이름**을 쓴다 → 어떤 코드가 어떤 제출을 만들었는지 1:1 대응.
...
## 3. 코드 (py)
### 3-1. 제출 재현 스크립트 — `src/submissions/approachN_vK_YYYYMMDD_HHMM.py`
- **제출 csv 하나 = 스크립트 하나.** 같은 이름의 `submissions/….csv`를 만드는 것이 유일한 역할.
- 내용은 "설정"만 담는다: 피처 버전, 파라미터 세트, 후처리 옵션, 출력 이름. 로직은 넣지 않고 `src/common/make_submission.py`의 `main()`을 호출한다.
- 파일 머리(docstring)에 반드시: 접근법·버전 한 줄 설명, 재현 명령, 사용한 설정, 정직 CV·LB 점수, 설명 문서(`team/approaches/approachN.md`) 경로.
- 실행: 루트에서 `python3 src/submissions/<이름>.py` → `submissions/<이름>.csv` 생성 (+ `submission.csv` 갱신, 쌍둥이 규칙본은 `experiments/submissions/twin_rule/`로).
- 새 버전을 만들 때: 기존 스크립트를 복사해 이름(버전·시각)을 바꾸고 설정만 수정한다. 기존 스크립트·csv는 지우지 않는다.
- 목록은 `src/submissions/README.md`에 한 줄씩 추가한다.

템플릿:
```python
"""접근 2 v2: 지식 피처 + 클래스 배율 후처리 (정직 CV 0.4691+0.012, LB 0.43)

재현: python3 src/submissions/approach2_v2_20260909_1653.py  →  submissions/approach2_v2_20260909_1653.csv
...
sys.argv = ["make_submission.py", "--features", "v4", "--class-scale", "experiments/2026-09-09_v4_xgb_grp",
            "--tag", "approach2_v2_20260909_1653"]
make_submission.main()
```

### 3-2. 공용 라이브러리 — `src/common/`
...
- 새 접근법의 로직은 `src/common/approachN_<이름>/`에 넣고, 진입 스크립트는 그 로직을 옵션으로 호출한다.
- 실행 시 `PYTHONPATH=src/common` (진입 스크립트는 스스로 path를 추가한다).

## 4. 제출 파일 (csv)
- `submissions/approachN_vK_YYYYMMDD_HHMM.csv` — `src/submissions/`의 py와 동일 이름. `submissions/submission.csv`는 가장 최근 파일 복사본.
